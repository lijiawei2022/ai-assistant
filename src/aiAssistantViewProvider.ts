...
## 任务
根据学生的学习记录，评估其当前水平，识别知识薄弱点，并推荐个性化的学习路径。

## 输出格式

### 📊 当前水平评估
根据交互记录，评估学生的编程水平（初学者/入门/进阶），并说明判断依据。

### 🔍 知识薄弱点
列出学生需要加强的知识领域，按优先级排列。
...
export class AiAssistantViewProvider implements vscode.WebviewViewProvider {
    ...
    private async startModelServer(): Promise<boolean> {
        ...
        const mergedModel = path.join(this._context.extensionPath, 'finetune', 'merged_model');
        const baseModel = path.join(this._context.extensionPath, 'finetune', 'base_model');
        let modelArg = '';
        if (fs.existsSync(mergedModel)) {
            modelArg = '';
            console.log('[LLM]使用微调模型 (merged_model)');
        } else if (fs.existsSync(baseModel)) {
            modelArg = '--base_only';
            console.log('[LLM]使用基础模型 (base_model)');
        } else {
            console.log('[LLM]未找到模型文件，无法启动服务');
            return false;
        }

        try {
            const args = [serveScript, '--port', String(LLM_PORT)];
            if (modelArg) {
                args.push(modelArg);
            }
    ...
    private async handleSyntaxCheck() {
        ...
        try {
            ...
            this.conversationHistory.push({ role: 'user', content: `[语法检查] ${fileName}` });
            this.conversationHistory.push({ role: 'assistant', content: rawAnswer });
    ...
    private async handleLearningPath() {
        ...
        try {
            ...
            let userContent = `请根据以下学习记录，为我推荐个性化的C语言学习路径：\n\n${historySummary}`;
            ...
            this.conversationHistory.push({ role: 'user', content: '[学习路径推荐] 请分析我的学习情况并推荐路径' });
            this.conversationHistory.push({ role: 'assistant', content: rawAnswer });
